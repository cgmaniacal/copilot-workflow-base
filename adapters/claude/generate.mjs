 ...
 */
function buildSettings(overlayConfig = {}) {
 ...
 * @returns {string}
 */
function buildClaudeMd(coreDir, agentDocsDir, overlayConfig = {}) {
  const detectedStacks = overlayConfig.detectedStacks || [];
  const sections = [];
 ...
 * @param {object} overlayConfig - Overlay configuration object (detectedStacks, branchConfig, superpowersInstalled, etc.)
 * @returns {{ claudeMdPath: string, settingsPath: string, copiedFiles: string[], paths: string[] }}
 */
export function generateClaudeConfig(targetDir, coreDir, agentDocsDir, overlayConfig = {}) {
  ...
  return { claudeMdPath, settingsPath, copiedFiles, paths: [claudeMdPath, settingsPath] };
}
